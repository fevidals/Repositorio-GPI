...
uga_admin <- readRDS("data/out/uga-admin-construct.RDS")
uga_officer <- readRDS("data/out/uga-officer-construct.RDS")
...
for (var in outcomes_citizen) {
  ...
  missing_prop <- uga_citizen %>% summarize(across(all_of(var), ~mean(is.na(.)), .names = "missingness_proportion"))
...
for (var in outcomes_officer) {
  ...
  # calculate proportion missing
  missing_prop <- uga_officer %>% summarize(across(all_of(var), ~mean(is.na(.)), .names = "missingness_proportion"))
...
for (var in outcomes_citizen_no_baseline) {
  # treatment effect estimation
  est <- lm_robust(
    as.formula(glue::glue("{ var } ~ Z_common + as.factor(block_ID)")),
    clusters = station_id,
    data = uga_citizen) %>%
    tidy %>%
    filter(term == "Z_common")
  ...
  # test for differential attrition
  missing_est <- lm_robust(
    as.formula(glue::glue("is.na({ var }) ~ Z_common + as.factor(block_ID)")),
    clusters = station_id,
    data = uga_citizen
  ) %>%
    tidy %>%
...
for (var in outcomes_admin) {
  ...
  # calculate proportion missing
  missing_prop <- uga_admin %>% summarize(across(all_of(var), ~mean(is.na(.)), .names = "missingness_proportion"))
...
saveRDS(estimates_df,  file = "data/out/uga-estimates.RDS")
