...
for (var in outcomes_citizen) {
  ...
  est <- lm_robust(
    as.formula(glue::glue("{ var } ~ Z + baseline_var + baseline_var_NA + as.factor(police_zones)")),
  ...
  missing_est <- lm_robust(
    as.formula(glue::glue("is.na({ var }) ~ Z + baseline_var + baseline_var_NA + as.factor(police_zones)")),
    clusters = communities,
    weights = 1 / S_communities_inclusion_prob * S_citizens_inclusion_prob,
    data = estimation_df
  ) %>%
  ...
  missing_prop <- lbr_citizen %>% summarize(across(all_of(var), ~mean(is.na(.)), .names = "missingness_proportion"))
...
for (var in c("crime_victim_idx_common")) {
  ...
  est_treatment <- lm_robust(
    as.formula(glue::glue("I({ var } - {var}_baseline) ~ as.factor(police_zones)")),
    # clusters = communities,
    subset = Z == 1,
    weights = 1 / S_communities_inclusion_prob * S_citizens_inclusion_prob,
    data = estimation_df
...
for (var in outcomes_admin) {
  ...
  est <- lm_robust(
    as.formula(glue::glue("{ var } ~ Z + baseline_var + baseline_var_NA + as.factor(police_zones)")),
    weights = 1 / S_communities_inclusion_prob,
    data = estimation_df) %>%
    tidy %>%
    filter(term == "Z")
  ...
  missing_prop <- 
    lbr_admin %>% 
    summarize(across(all_of(var), ~mean(is.na(.)), .names = "missingness_proportion"))
...
saveRDS(estimates_df,  file = "data/out/lbr-estimates.RDS")
