...
for (var in outcomes_citizen) {
  ...
  estimates_list[[paste0(var, "_het_trust")]] <- iv_robust(
    as.formula(glue::glue("{ var } ~ groupformed_presence_201810 * (baseline_het_var_trust + baseline_het_var_trust_NA) + baseline_var + baseline_var_NA + as.factor(muncode) | 
            Z  * (baseline_het_var_trust + baseline_het_var_trust_NA) + baseline_var + baseline_var_NA + as.factor(muncode)")),
    clusters = meeting_id, 
    data = estimation_df) %>%
    tidy %>%
    filter(term == "groupformed_presence_201810:baseline_het_var_trust")
  
  # treatment effect estimation
  estimates_list[[paste0(var, "_het_trustcom")]] <- iv_robust(
    as.formula(glue::glue("{ var } ~ groupformed_presence_201810 * (baseline_het_var_trustcom + baseline_het_var_trustcom_NA) + baseline_var + baseline_var_NA + as.factor(muncode) | 
            Z  * (baseline_het_var_trustcom + baseline_het_var_trustcom_NA) + baseline_var + baseline_var_NA + as.factor(muncode)")),
    clusters = meeting_id, 
    data = estimation_df) %>%
    tidy %>%
    filter(term == "groupformed_presence_201810:baseline_het_var_trustcom")
  
  # treatment effect estimation
  estimates_list[[paste0(var, "_het_crime")]] <- iv_robust(
    as.formula(glue::glue("{ var } ~ groupformed_presence_201810 * (baseline_het_var_crime + baseline_het_var_crime_NA) + baseline_var + baseline_var_NA + as.factor(muncode) | 
            Z  * (baseline_het_var_crime + baseline_het_var_crime_NA) + baseline_var + baseline_var_NA + as.factor(muncode)")),
    clusters = meeting_id, 
    data = estimation_df) %>%
    tidy %>%
    filter(term == "groupformed_presence_201810:baseline_het_var_crime")
...
saveRDS(estimates_df,  file = "data/out/bra-estimates-het.RDS")
